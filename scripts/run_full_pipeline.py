...
def main():
    ...
    p.add_argument("--dataset-name", default=None, help="Name of the dataset (e.g. 'openstack'). Used to create a subfolder in data/ for artifacts.")
    ...
    p.add_argument("--llm-validate", action="store_true", help="Enable Optional LLM (Ollama) validation step (Slow but accurate)")
    args = p.parse_args()
    ...
    if args.dataset_name:
        ...
        print(f"Using dataset-specific output directory: {data_root}")
    else:
        data_root = Path("data")
    ...
    parsed_path = data_root / "parsed_sample.csv"
    templates_path = data_root / "parsed_templates.csv"
    ...
    for i in range(len(raw_df)):
        ...
        # Pass 'service' if it exists in the input CSV (e.g. from ingest_folder.py)
        extra_meta = {}
        if "service" in row and row["service"]:
            extra_meta["service"] = row["service"]
            
        parser.process_line(raw_line, i + 1, extra_meta=extra_meta)
    ...
    # Only apply if 'service' is missing or looks invalid (e.g. numeric/short)
    if 'raw' in parsed_df.columns:
        # Check if we need to refine service
        needs_refinement = True
        if 'service' in parsed_df.columns:
            # If service is already populated (e.g. from extra_meta), check if it looks valid
            # Heuristic: if it's longer than 3 chars and not numeric, assume it's good
            sample_svc = parsed_df['service'].dropna().iloc[0] if not parsed_df['service'].dropna().empty else ""
            if len(str(sample_svc)) > 3 and not str(sample_svc).isdigit():
    ...
    cleaned_path = data_root / "cleaned_templates.csv"
    ...
    merged_out = data_root / "merged_templates.csv"
    ...
    embeddings_out_dir = data_root / "edges"
    ...
    # 7.5) Validation (Mandatory Cross-Encoder Step)
    # We now run this by default to ensure high-quality edges (Thesis Requirement).
    # The --validate flag is deprecated but kept for backward compatibility (it's now always True).
    ...
    # Filter edges with low validation scores to improve Precision
    # We use a lenient threshold (0.0) because the Cross-Encoder is uncalibrated logits or raw scores
    # depending on implementation. If it returns probabilities (0-1), 0.5 is a good default.
    # Assuming SemanticValidator returns probabilities or we just want to re-rank.
    # For now, we pass all edges but with updated scores.
    
    # Re-create generator
    gen = (e for e in validated_edges)
    manifest["events"].append({"ts": time.time(), "event": "validation_completed", "count": len(candidates)})

    # 7.6) Optional LLM Judge Step
    if getattr(args, "llm_validate", False):
        print("LLM Validation enabled: Analyzing causal links with LLM Judge...")
        from src.validation import LLMValidator
        try:
            llm_validator = LLMValidator()
            # We re-consume the validated_edges from the previous step
            validated_edges = llm_validator.validate_candidates(validated_edges)
            gen = (e for e in validated_edges)
            manifest["events"].append({"ts": time.time(), "event": "llm_validation_completed", "count": len(validated_edges)})
        except Exception as e:
            print(f"Warning: LLM Validation failed or Ollama not running: {e}")
            print("Proceeding with Cross-Encoder results only.")
    ...
    store = EdgeStore(db_path, merged_templates_path=str(merged_out))
    store.init_db()
    ...
    try:
        ...
        manifest_dir = data_root / "run_manifests"
