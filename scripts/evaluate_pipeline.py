...
def load_gold_standard_csv(csv_path: Path) -> Set[Tuple[str, str]]:
    """
    Load ground truth edges directly from a pre-computed CSV.
    Expected format: source,target,weight,type,...
    """
    print(f"Loading gold standard edges from {csv_path}...")
    if not csv_path.exists():
        raise FileNotFoundError(f"Gold standard CSV not found at {csv_path}")
        
    df = pd.read_csv(csv_path)
    edges = set()
    for _, row in df.iterrows():
        if pd.notna(row['source']) and pd.notna(row['target']):
            edges.add((str(row['source']), str(row['target'])))
            
    print(f"Loaded {len(edges)} unique edges from gold standard CSV.")
    return edges
...
def get_static_ground_truth(dataset_name: str = "openstack") -> Set[Tuple[str, str]]:
    """
    Returns a set of architecturally known dependencies for the given dataset.
    ...
    filename = f"{dataset_name}_architecture.json"
        ...
        try:
            with open(kb_path, "r") as f:
                ...
                if isinstance(data, dict) and "dependencies" in data:
                    # Format: {"dependencies": [{"source": "A", "target": "B"}, ...]}
                    edges = set()
                    ...
                    # Format: [["A", "B"], ...] (Legacy OpenStack format)
                    return {tuple(edge) for edge in data}
                else:
                    print(f"Warning: Unknown format in {kb_path}")
                    return set()
...
def calculate_metrics(ground_truth: Set[Tuple[str, str]], inferred: Set[Tuple[str, str]], dataset_name: str = "openstack") -> Dict[str, float]:
    """Calculate Precision, Recall, and F1 using Hybrid Ground Truth."""
    
    # 1. Load Static Architecture Truth
    static_gt = get_static_ground_truth(dataset_name)
...
def main():
    ...
    parser.add_argument("--parsed-logs", type=Path, default=None, help="Path to parsed logs CSV (optional if --gold-standard provided)")
    parser.add_argument("--gold-standard", type=Path, default=None, help="Path to pre-computed gold standard CSV")
    ...
    parser.add_argument("--dataset-name", type=str, default="openstack", help="Name of the dataset (e.g. openstack)")
    ...
    try:
        if args.gold_standard:
             ground_truth = load_gold_standard_csv(args.gold_standard)
        elif args.parsed_logs:
            df = load_parsed_logs(args.parsed_logs)
            ground_truth = generate_ground_truth(df)
        else:
             # Default fallback if neither provided? Or error?
             # For backward compatibility, default to data/parsed_sample.csv if it exists?
             # But we kept default=None above.
             # Let's check a sensible default.
             default_logs = Path("data/parsed_sample.csv")
             if default_logs.exists():
                 print(f"No input provided, defaulting to {default_logs}...")
                 df = load_parsed_logs(default_logs)
                 ground_truth = generate_ground_truth(df)
             else:
                 print("Error: Must provide either --gold-standard or --parsed-logs")
                 return
    except Exception as e:
    ...
    # 3. Calculate Metrics
    metrics = calculate_metrics(ground_truth, inferred_set, args.dataset_name)
    ...
    if metrics['fn'] > 0:
        ...
        static_gt = get_static_ground_truth(args.dataset_name)
        valid_ground_truth = ground_truth.union(static_gt)
    ...
    if args.suggest_updates:
        suggest_updates(inferred_dict, ground_truth, get_static_ground_truth(args.dataset_name))
