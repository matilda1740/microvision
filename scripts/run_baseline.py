...
def load_data(input_path: Path) -> pd.DataFrame:
    ...
    if "template" not in df.columns:
        ...
        if "semantic_text" in df.columns:
            print("Warning: 'template' column missing, using 'semantic_text'")
            df["template"] = df["semantic_text"]
...
def compute_tfidf_edges(df: pd.DataFrame, top_k: int = 10, threshold: float = 0.2) -> Generator[Dict[str, Any], None, None]:
    ...
    for i in range(n_samples):
        ...
        for j in top_indices:
            ...
            # EdgeStore expects target metadata to extract service name
            tgt_meta = {
                "service": str(target_row.get("service", "")),
                "component": str(target_row.get("component", ""))
            }
...
def main():
    ...
    # 2. Init DB
    store = EdgeStore(str(args.db), merged_templates_path=str(args.input))
    store.init_db()
    if args.clear_db:
