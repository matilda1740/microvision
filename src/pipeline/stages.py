...
def reservoir_sample_csv(path: Path, sample_size: int, seed: int = 42, contiguous: bool = False) -> List[Dict[str, Any]]:
    ...
    with path.open("r", encoding="utf-8", errors="ignore") as fh:
        ...
        try:
            start = fh.read(8192)
            fh.seek(0)
            # Heuristic: if it starts with "service,raw", it's our preprocessed CSV
            if start.startswith("service,raw"):
                has_header = True
            else:
...
def compute_embeddings_for_use_df(use_df: pd.DataFrame, model_name: str | None, device: str | None, output_dir: str | None = None, batch_size: int = 128, num_workers: int = 1) -> tuple[list[list[float]], pd.DataFrame, list[str]]:
    ...
    """
    # Dynamic Context Injection (Tier 1 + Dynamic Keywords)
    # 1. Extract top keywords per service using TF-IDF logic (simplified)
    from collections import Counter
    import re

    def get_service_keywords(df: pd.DataFrame) -> Dict[str, str]:
        service_keywords = {}
        if "service" not in df.columns:
            return service_keywords

        # Stopwords to ignore
        stopwords = {"the", "to", "in", "of", "a", "is", "for", "on", "at", "by", "with", "from", "and", "or", "ts", "service", "info", "warn", "error", "debug", "trace", "pid", "id", "req_id", "main", "root", "started", "starting"}

        for service, group in df.groupby("service"):
            if not isinstance(service, str):
                continue
            
            # Aggregate all semantic text for this service
            all_text = " ".join(group["semantic_text"].fillna("").astype(str).tolist()).lower()
            # Tokenize
            tokens = re.findall(r"\b[a-z]{3,}\b", all_text)
            # Filter stopwords
            tokens = [t for t in tokens if t not in stopwords]
            # Get top 5 most frequent words
            most_common = [word for word, count in Counter(tokens).most_common(5)]
            service_keywords[service] = " ".join(most_common)
        
        return service_keywords

    # 2. Apply injection
    service_keywords_map = get_service_keywords(use_df)
    
    def inject_context(row):
        text = str(row.get("semantic_text", ""))
        service = row.get("service")
        
        if service and isinstance(service, str) and service in service_keywords_map:
            keywords = service_keywords_map[service]
            # Format: "[service_name | keyword1 keyword2] original_log"
            return f"[{service} | {keywords}] {text}"
        elif service and isinstance(service, str):
             return f"[{service}] {text}"
        return text

    # Apply to a copy to avoid mutating the original dataframe in a way that affects other stages unexpectedly
    # But we need the embeddings to match these documents.
    # The 'encode_templates' function uses 'semantic_text' column.
    # So we must update it in place or pass a modified df.
    
    # We'll modify use_df in place as it's a local copy in the pipeline flow usually
    if "service" in use_df.columns:
        print("Applying Dynamic Context Injection to semantic_text...")
        use_df["semantic_text"] = use_df.apply(inject_context, axis=1)
...
def compute_and_persist_edges(use_df: pd.DataFrame, embeddings: list[list[float]], collection, db_path: str, top_k: int, threshold: float, alpha: float, clear_db: bool = False) -> int:
    ...
    if getattr(settings, "ENABLE_LLM_VALIDATION", False):
        print("LLM Validation ENABLED: Intercepting edges for verification...")
        try:
            from src.validation import LLMValidator
            validator = LLMValidator()

            def llm_wrapper(upstream_gen):
                batch = []
                for edge in upstream_gen:
                    batch.append(edge)
                    # Small batch size for LLM to show frequent progress
                    if len(batch) >= 5:
                        batch = validator.validate_candidates(batch)
                        yield from batch
                        batch = []
                if batch:
                    batch = validator.validate_candidates(batch)
            ...
            print(f"Warning: LLM Validation enabled but failed to initialize: {e}")
