...
class EdgeStore:
    def __init__(self, db_path: str, merged_templates_path: Optional[str] = None):
        self.db_path = db_path
        self.merged_templates_path = merged_templates_path
    ...
    def init_db(self) -> None:
        ...
        try:
            ...
            if "llm_verification" not in cols:
                cur.execute("ALTER TABLE edges ADD COLUMN llm_verification TEXT")
            if "llm_confidence" not in cols:
                cur.execute("ALTER TABLE edges ADD COLUMN llm_confidence REAL")
            self.conn.commit()
        except Exception:
        ...
        try:
            ...
            mt_path = self.merged_templates_path if self.merged_templates_path else "data/merged_templates.csv"
            ...
            if _pd and _pd.io.common.file_exists(mt_path):
                ...
                logger.warning(f"EdgeStore: Could not find merged templates at {mt_path}")
                self._merged_comp_map = {}
                self._merged_serv_map = {}
        except Exception as e:
            logger.warning(f"EdgeStore: Error loading merged templates: {e}")
            self._merged_comp_map = {}
            self._merged_serv_map = {}
